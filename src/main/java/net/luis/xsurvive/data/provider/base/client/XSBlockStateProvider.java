...
public class XSBlockStateProvider extends BlockStateProvider {
	...
	public XSBlockStateProvider(@NotNull DataGenerator generator, @NotNull ExistingFileHelper existingFileHelper) {
	...
	private void litFacingBlock(@NotNull Block block) {
	...
	private void attachedStemBlock(@NotNull Block block) {
		String name = Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(block)).getPath();
	...
	private void floorAndSideFireBlock(@NotNull Block block) {
		String name = Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(block)).getPath();
		// @formatter:off
		this.models().getBuilder(name + "_floor0")
			.parent(new ExistingModelFile(ResourceLocation.withDefaultNamespace("block/template_fire_floor"), this.existingFileHelper))
		...
		this.models().getBuilder(name + "_side0")
			.parent(new ExistingModelFile(ResourceLocation.withDefaultNamespace("block/template_fire_side"), this.existingFileHelper))
			.texture("fire", this.modLoc("block/" + name + "_0")).renderType("cutout");
		this.models().getBuilder(name + "_side1")
			.parent(new ExistingModelFile(ResourceLocation.withDefaultNamespace("block/template_fire_side"), this.existingFileHelper))
			.texture("fire", this.modLoc("block/" + name + "_1")).renderType("cutout");
		this.models().getBuilder(name + "_side_alt0")
			.parent(new ExistingModelFile(ResourceLocation.withDefaultNamespace("block/template_fire_side_alt"), this.existingFileHelper))
			.texture("fire", this.modLoc("block/" + name + "_0")).renderType("cutout");
		this.models().getBuilder(name + "_side_alt1")
			.parent(new ExistingModelFile(ResourceLocation.withDefaultNamespace("block/template_fire_side_alt"), this.existingFileHelper))
			.texture("fire", this.modLoc("block/" + name + "_1")).renderType("cutout");
		this.models().getBuilder(name + "_up0")
			.parent(new ExistingModelFile(ResourceLocation.withDefaultNamespace("block/template_fire_up"), this.existingFileHelper))
			.texture("fire", this.modLoc("block/" + name + "_0")).renderType("cutout");
		this.models().getBuilder(name + "_up1")
			.parent(new ExistingModelFile(ResourceLocation.withDefaultNamespace("block/template_fire_up"), this.existingFileHelper))
			.texture("fire", this.modLoc("block/" + name + "_1")).renderType("cutout");
		this.models().getBuilder(name + "_up_alt0")
			.parent(new ExistingModelFile(ResourceLocation.withDefaultNamespace("block/template_fire_up_alt"), this.existingFileHelper))
			.texture("fire", this.modLoc("block/" + name + "_0")).renderType("cutout");
		...
		this.getMultipartBuilder(block)
				...
				.condition(MysticFireBlock.UP, false)
				.end()
				.end().part().modelFile(this.blockModel(block, "up0")).nextModel()
				.modelFile(this.blockModel(block, "up1")).nextModel()
				.modelFile(this.blockModel(block, "up_alt0")).nextModel()
				.modelFile(this.blockModel(block, "up_alt1")).addModel()
	...
	private @NotNull ModelFile blockModel(@NotNull Block block) {
		return this.blockModel(block, null);
	}
	
	private @NotNull ModelFile blockModel(@NotNull Block block, @Nullable String addition) {
		ResourceLocation name = Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(block));
		if (addition == null || addition.isEmpty()) {
			return new UncheckedModelFile(ResourceLocation.fromNamespaceAndPath(name.getNamespace(), "block/" + name.getPath()));
		}
		return new UncheckedModelFile(ResourceLocation.fromNamespaceAndPath(name.getNamespace(), "block/" + name.getPath() + "_" + addition));
	}
